...
//		for(int i=0;i<itemList.size();i++){
//			if(itemList.get(i).isSelected()){
//				 onSelectListener.selecting(itemList.get(i).id, itemList.get(i).itemText);
//			}
//		}
	}
...
//		for(int i=0;i<itemList.size();i++){
//			if(itemList.get(i).isSelected()){
//				return itemList.get(i).itemText;
//			}
//		}
		return "";
	}
	...
	public void setDefault(int index) {
		...
		if(itemList.size()>index){
			...
			move = itemList.get(0).moveToSelected();
		}
	...
	private class ItemObject {
		...
		public void drawSelf(Canvas canvas) {
			...
			if (isSelected()) {
				...
				// 计算当前字体大小
				if(MyApplication.isModifyTextSize==1){
